...
interface SearchContent {
  ...
  empty_state_button_label: string;
}

function SearchResultsInner({ content }: { content: SearchContent }) {
  ...
  return (
    <div className="bg-slate-50 pb-24 pt-40">
      <div className="container max-w-3xl">
        <p className="text-sm font-semibold uppercase tracking-wide text-red-600">{content.label}</p>
        ...
        {results.length === 0 && (
          <div className="mt-16 rounded-3xl bg-white p-10 text-center shadow-card">
            <p className="text-navy-500">{content.empty_state_message}</p>
            <div className="mt-6">
              <GradientButton href="/appointment">{content.empty_state_button_label}</GradientButton>
            </div>
          </div>
        )}
      </div>
...
export function SearchResultsClient({ content }: { content: SearchContent }) {
  return (
    <Suspense>
      <SearchResultsInner content={content} />
    </Suspense>
  );
}
